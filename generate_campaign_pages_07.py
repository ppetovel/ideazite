...
# Genera paginas de campañas desde CSV (con slug: Title + Brand + Agency + Year)
# y crea un CSV nuevo con columna "Permalink".
# Ahora incluye todas las columnas dinámicamente (excepto "Permalink").
# Salida de consola en ASCII (sin símbolos raros).

import csv, os, re, unicodedata, argparse, html, pathlib, sys, shutil, urllib.parse

# -------- Config por defecto (se pueden sobrescribir por CLI) --------
...
MAX_SLUG_LEN = 120  # Limite conservador para evitar rutas largas en Windows
...
def render_board_link(url: str) -> str:
    ...
    <figure style="margin:18px 0;">
        <a href="{html_escape(url)}" target="_blank" rel="noopener">
            <img src="{html_escape(url)}" alt="Board image" style="max-width:100%;height:auto;border:1px solid #ccc;">
        </a>
        <figcaption style="font-size:13px;color:#666;margin-top:6px;text-align:center;">
            View Board
        </figcaption>
    </figure>'''
...
def generate_pages_and_permalink_csv(csv_path: str, permalink_csv_out: str, clean: bool = False):
    ...
    for idx, row in enumerate(rows, 1):
        try:
            ...
            # Generar tabla dinámica con todas las columnas (menos Permalink y Board image)
            table_rows = []
            for col, val in row.items():
                if col.lower().strip() in ("permalink", "board image"):
                    continue

                col_name = html_escape(col)
                v = (val or "").strip()

                # Si el valor es un link (http/https), mostrar texto "Link" clickeable en nueva pestaña
                if re.match(r"^https?://", v, flags=re.IGNORECASE):
                    col_val_html = f'<a href="{html_escape(v)}" target="_blank" rel="noopener">Link</a>'
                else:
                    col_val_html = html_escape(v)

                table_rows.append(f"<tr><th>{col_name}</th><td>{col_val_html}</td></tr>")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Genera paginas de campañas desde CSV y crea CSV con Permalink.")
