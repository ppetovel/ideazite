...
# Genera paginas de campañas (gris) desde CSV con slug: Title + Brand + Agency + Year
# y crea un CSV nuevo con columna "Permalink" (ruta relativa).
# Salida de consola en ASCII (sin simbolos raros).

import csv, os, re, unicodedata, argparse, html, pathlib, sys, shutil, urllib.parse

# -------- Defaults (Gerety 2025) --------
FESTIVAL     = "Gerety Awards"
YEAR         = "2025"
SITE_BASE    = "https://ideazite.com"
YEAR_ROOT    = "/awards/geretyawards/2025/"
OUTPUT_ROOT  = "./awards/geretyawards/2025/campaigns"  # carpeta de salida
...
def render_board_link(url: str) -> str:
    ...
    if not u or not re.match(r'^https?://', u, flags=re.I):
        return ""
    return f'<p><a href="{html_escape(u)}" target="_blank" rel="noopener">View Board</a></p>'
...
def generate_pages_and_permalink_csv(csv_path: str, permalink_csv_out: str, clean: bool = False):
    ...
    out_rows = []
    for row in rows:
        # Campos segun encabezados brindados:
        # Festival  Year  Track  Title  Brand  Agency  Category  Format  Award level  Board image
        title       = first_nonempty(row, ["Title"])
        brand       = first_nonempty(row, ["Brand"])
        agency      = first_nonempty(row, ["Agency"])
        track       = first_nonempty(row, ["Track"])
        category    = first_nonempty(row, ["Category"])
        award_level = first_nonempty(row, ["Award level", "Award Level"])
        board_image = first_nonempty(row, ["Board image", "Board Image"])
        year_val    = first_nonempty(row, ["Year"]) or YEAR

        # Saltar filas totalmente vacias
        if not any([title, brand, agency, track, category, award_level, board_image, year_val]):
            continue
